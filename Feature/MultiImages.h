...
public:
    ...
    const vector<vector<vector<Point2> > >          & getFeatureMatches(const vector<pair<int, int> >& images_match_graph_pair_list) const;
    ...
    struct Race_Agv{
        const MultiImages *pp;
        pair<int,int> Match_pair;
    };
private:    
    /*** Debugger ***/
