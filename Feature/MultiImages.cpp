...
MultiImages::MultiImages(const string & _file_name,
    ...
    // cout << images_data.size() << endl;
    int uu_core=THREADS;
    if(parameter.image_file_full_names.size()<THREADS)
        uu_core = parameter.image_file_full_names.size();
 #pragma omp parallel for num_threads(uu_core)
    for(int i = 0; i < parameter.image_file_full_names.size(); ++i) {

        images_data[i].imageData(parameter.file_dir, parameter.image_file_full_names[i], _width_filter, _length_filter);
        vector<Point2> useless = images_data[i].getFeaturePoints();
        images_pack[i] = images_data[i].img;
    }
    // 图片加载完成
    vector<vector<bool>> stitch_order = Stitch_Order(images_pack);  //auto order
    parameter.images_match_graph_automatically_ = stitch_order;
    this->images_match_graph_pair_list = parameter.getImagesMatchGraphPairList();
}
void* MultiImages::thread_fun(void* agv){
    Race_Agv *obj_agv = (Race_Agv*)agv;
    pair<int, int> match_pair = obj_agv->Match_pair;
    const vector<pair<int, int> > & initial_indices = obj_agv->pp->getInitialFeaturePairs(match_pair);
    const vector<Point2> & m1_fpts = obj_agv->pp->images_data[match_pair.first ].getFeaturePoints();
    const vector<Point2> & m2_fpts = obj_agv->pp->images_data[match_pair.second].getFeaturePoints();
    vector<Point2> X, Y;
    ...
    vector<pair<int, int> > & result = obj_agv->pp->feature_pairs[match_pair.first][match_pair.second];
    result = obj_agv->pp->getFeaturePairsBySequentialRANSAC(match_pair, X, Y, initial_indices);
...
void MultiImages::doFeatureMatching() const {
    ...
    // const vector<vector<vector<Point2> > > & feature_matches = getFeatureMatches(images_match_graph_pair_list);  // 耗时点
...
const vector<Point2> & MultiImages::getImagesLinesProject(const int _from, const int _to) const {
    ...
    if(images_lines_projects[_from][_to].empty()) {
        ...
        const vector<vector<vector<Point2> > >& feature_matches = this->feature_matches;
...
const vector<vector<vector<pair<int, int> > > > & MultiImages::getFeaturePairs() const {
    if(feature_pairs.empty()) {
        ...
        for(int i = 0; i < images_match_graph_pair_list.size(); ++i){
            pair<int, int> temp = make_pair(images_match_graph_pair_list[i].first,images_match_graph_pair_list[i].second);
            match_pairs.emplace_back(temp);
        }
//        pthread_t tid[4];
//        for(int count = 0;count<4;count++){
//            Race_Agv agv = {this,match_pairs[count]};
//            int ret = pthread_create(&tid[count], NULL, thread_fun,(void*)&agv);
//            if(ret != 0){
...
//        pthread_exit(NULL);
 ...
 #pragma omp parallel for num_threads(max_thread)
        for(int i = 0; i < match_pairs.size(); ++i) {
            // const pair<int, int> &match_pair = images_match_graph_pair_list[i];
            pair<int, int> match_pair(match_pairs[i]);
            //// 耗时片段 ////
            const vector<pair<int, int> >  &initial_indices = getInitialFeaturePairs(match_pair);  // sift
            ...
            //////// **** ////////
            // cout<< initial_indices.size()<<"  "<< m1_fpts.size()<<"  " <<m2_fpts.size()<<endl;
            vector<Point2> X, Y;
            X.reserve(initial_indices.size());
            Y.reserve(initial_indices.size());
...
const vector<vector<vector<Point2> > > & MultiImages::getFeatureMatches(const vector<pair<int, int> >& images_match_graph_pair_list) const {
...
vector<pair<int, int> > MultiImages::getFeaturePairsBySequentialRANSAC(const pair<int, int> & _match_pair,
    ...
    cout<<"pair: "<<_match_pair.first<<" -- "<<_match_pair.second<<"   size: "<<_X.size()<<" "<<_Y.size()<<endl;
...
vector<pair<int, int> > MultiImages::getInitialFeaturePairs(const pair<int, int> & _match_pair) const {
 ...
 #pragma omp parallel for num_threads(THREADS)
    ...
    //cout<<" feature "<<feature_pairs_result.size()<<endl;
    double mean, std;
...
#endif
    // cout<<" ini: " <<initial_indices.size()<<endl;
...
Mat MultiImages::textureMapping(const vector<vector<Point2> > & _vertices,
    ...
    _warp_images.resize(images_data.size());
    origins.resize(images_data.size());
    new_weight_mask.resize(images_data.size());
...
#pragma omp parallel for num_threads(THREADS)
    for(int i = 0; i < images_data.size(); ++i) {
        ...
        _warp_images[i] = image;
        origins[i] = Point2f(rects[i].x, rects[i].y);
        if (_blend_method != BLEND_AVERAGE) {
            new_weight_mask[i] = w_mask;
        }
    }
